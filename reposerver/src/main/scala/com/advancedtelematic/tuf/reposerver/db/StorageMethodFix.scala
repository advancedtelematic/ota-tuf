...
class StorageMethodFix(implicit
                       val db: Database,
  ...
  def needsFix(item: TargetItem): Boolean =
    item.uri.toString().contains("s3.eu-central-1.amazonaws.com") && item.storageMethod == StorageMethod.Unmanaged

  def fixTargetItemIfNeeded = Flow[TargetItem].mapAsync(3) { item =>
    if(needsFix(item))
      db.run(targetItems.filter(_.repoId === item.repoId).filter(_.filename === item.filename).map(_.storageMethod).update(StorageMethod.Managed))
        .map { _ =>
  ...
  def run: Future[Done] = {
    val dbStream = db.stream(targetItems.filter(_.storageMethod === StorageMethod.Unmanaged).result)
