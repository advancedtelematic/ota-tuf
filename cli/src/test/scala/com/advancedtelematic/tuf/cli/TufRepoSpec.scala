...
import com.advancedtelematic.libtuf.data.TufDataType.{Ec25519KeyType, Ec25519TufKeyPair, Ec25519TufPrivateKey, RoleType, SignedPayload, TargetFormat, TargetName, TargetVersion, TufKey}
...
class TufRepoSpec extends CliSpec {
  ...
  def rotate(repo: TufRepo, reposerverClient: FakeUserReposerverClient = new FakeUserReposerverClient): Future[(TufKey, TufKey, SignedPayload[RootRole])] = {
    ...
    val pub = repo.genKeys(newRootName, Ec25519KeyType, 256).get.pubkey
    val pubT = repo.genKeys(newTargetsName, Ec25519KeyType, 256).get.pubkey
  ...
  test("adds a key to a repo") {
    ...
    repo.genKeys(KeyName("newkey"), Ec25519KeyType, 256)
  ...
  test("saves deleted root when rotating") {
    ...
    oldPrivateKey shouldBe a[Ec25519TufPrivateKey]
  }
  ...
  test("bumps version when signing targets role") {
    ...
    repo.genKeys(targetsKeyName, Ec25519KeyType, 256).get
  ...
  test("signs targets") {
    ...
    val targetsKeyName = KeyName("somekey")
    val pub = repo.genKeys(targetsKeyName, Ec25519KeyType, 256).get.pubkey
