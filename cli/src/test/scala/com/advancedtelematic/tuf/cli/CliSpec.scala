...
import com.advancedtelematic.libtuf.data.TufDataType.{Ec25519KeyType, Ec25519TufKeyPair, KeyId, RoleType, SignedPayload, TufKey, TufKeyPair, TufPrivateKey}
...
class FakeUserReposerverClient extends UserReposerverClient {
  ...
  override def fetchKeyPair(keyId: KeyId): Future[TufKeyPair] =
    Future.successful(Ec25519TufKeyPair(oldPublicKey, oldPrivateKey)).filter(_.pubkey.id == keyId)
}
