...
import com.advancedtelematic.libtuf.data.TufDataType.{Ed25519KeyType, Ed25519TufKey, Ed25519TufPrivateKey, KeyType, RSATufKey, RSATufPrivateKey, RsaKeyType, SignedPayload, TufKey, TufPrivateKey}
...
class RepoManagementSpec extends CliSpec {
  ...
  test("credentials.zip without tufrepo.url throws proper error") {
    val repoT = RepoManagement.initialize(randomName, randomRepoPath, credentialsZipNoTufRepoEd25519)
  ...
  test("throws error for already initialized repos") {
    ...
    val repoT = RepoManagement.initialize(randomName, path, credentialsZipEd25519)
    repoT shouldBe a[Success[_]]

    val repoF = RepoManagement.initialize(randomName, path, credentialsZipEd25519)
    repoF shouldBe Failure(RepoAlreadyInitialized(path))
  }

  test("can initialize repo from ZIP file") {
    val repoT = RepoManagement.initialize(randomName, randomRepoPath, credentialsZipEd25519)
    repoT shouldBe a[Success[_]]
  }

  test("can initialize repo from ZIP file specifying custom repo") {
    val repoT = RepoManagement.initialize(randomName, randomRepoPath, credentialsZipEd25519, repoUri = Some(new URI("https://ats.com")))
    repoT shouldBe a[Success[_]]
  ...
  test("skips auth when no_auth: true") {
    val repoT = RepoManagement.initialize(randomName, randomRepoPath, credentialsZipNoAuthEd25519)
  ...
  def keySpecific[T <: KeyType, Pub <: T#Pub : ClassTag, Priv <: T#Priv : ClassTag](keyType: KeyType, name: String): Unit = {
    ...
    test("reads targets keys from credentials.zip if present " + name) {
      val repoT = RepoManagement.initialize(randomName, randomRepoPath, zip)
      repoT shouldBe a[Success[_]]

      val repo = repoT.get

      repo.authConfig.get.get shouldBe a[AuthConfig]
      parseFile(repo.repoPath.resolve("keys/targets.pub").toFile).flatMap(_.as[TufKey]).valueOr(throw _) shouldBe a[Pub]
      parseFile(repo.repoPath.resolve("keys/targets.sec").toFile).flatMap(_.as[TufPrivateKey]).valueOr(throw _) shouldBe a[Priv]
    }

    test("export includes root.json " + name) {
    ...
    test("creates base credentials.zip if one does not exist " + name) {
      val repo = RepoManagement.initialize(randomName, randomRepoPath, zip).get
