...
import com.advancedtelematic.libtuf.data.TufDataType.{Ec25519KeyType, Ec25519TufKey, Ec25519TufPrivateKey, SignedPayload, TufKey, TufPrivateKey}
...
class RepoManagementSpec extends CliSpec {
  ...
  test("reads targets keys from credentials.zip if present") {
    ...
    parseFile(repo.repoPath.resolve("keys/targets.pub").toFile).flatMap(_.as[TufKey]).valueOr(throw _) shouldBe a[Ec25519TufKey]
    parseFile(repo.repoPath.resolve("keys/targets.sec").toFile).flatMap(_.as[TufPrivateKey]).valueOr(throw _) shouldBe a[Ec25519TufPrivateKey]
  }
  ...
  test("export includes root.json") {
    ...
    repo.genKeys(KeyName("targets"), Ec25519KeyType, 256).get
  ...
  test("can export zip file") {
    val repo = RepoManagement.initialize(randomName, randomRepoPath, credentialsZip).get
    repo.genKeys(KeyName("default-key"), Ec25519KeyType, 256)
