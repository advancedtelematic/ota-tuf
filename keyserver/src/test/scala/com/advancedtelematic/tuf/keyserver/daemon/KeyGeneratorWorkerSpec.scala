...
class KeyGeneratorWorkerSpec extends TufKeyserverSpec with TestKitBase with DatabaseSpec with ImplicitSender
  ...
  test("generates a ed25519 key") {
    val keyGenReq = keyGenRequest(keyType = Ed25519KeyType).futureValue
    actorRef ! keyGenReq
    ...
    dbKey.keyType shouldBe Ed25519KeyType
    dbKey.publicKey.toPem should include("BEGIN PUBLIC KEY")

    TufCrypto.ed25519Crypto.parsePublic(Hex.toHexString(dbKey.publicKey.getEncoded)).get shouldBe a[Ed25519TufKey]
  }
