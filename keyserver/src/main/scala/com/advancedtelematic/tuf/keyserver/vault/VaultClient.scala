...
import com.advancedtelematic.libtuf.data.TufDataType.{EdKeyType, KeyId, KeyType, RSATufPrivateKey, TufKey, TufKeyPair, TufPrivateKey}
...
object VaultClient {
  ...
  object VaultKey {
    ...
    private val legacyEcPublicKeyDecoder = Decoder[String].emapTry { str ⇒
      EdKeyType.crypto.parsePublic {
        val base64Str =
          str
            .replace("-----BEGIN PUBLIC KEY-----","")
            .replace("-----END PUBLIC KEY-----", "")
            .replace("\n","")
        Hex.toHexString(Base64.decode(base64Str))
      }
    }

    private def lenientPublicKeyDecoder(keyType: KeyType): Decoder[TufKey] = TufCodecs.tufKeyDecoder or legacyPublicKeyDecoder(keyType).widen or legacyEcPublicKeyDecoder.widen
