...
import com.advancedtelematic.ota_tuf.data.DataType.{GroupId, KeyGenId, KeyGenRequest, Signature}
...
with KeyGenRequestSupport {
  ...
  test("root role payload must be signed with root key") {
    ...
    val rootKeyGenRequest = KeyGenRequest(KeyGenId.generate(),
      groupId, KeyGenRequestStatus.GENERATED, RoleType.ROOT)
    ...
    async {
      ...
      val signed = await(rootGeneration.findSigned(groupId))
